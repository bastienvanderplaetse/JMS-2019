...
class NEAT():
	def __init__(self, output_size):
		...
		try:
			doc = minidom.parse('neat_config.xml')
	...
	def _load_generation(self, start_with_gen):
		with open('gen/neat_gen_' + str(start_with_gen) + '.json') as f:
	...
	def _create_config_file(self):
		...
		doc.writexml(open('neat_config.xml', 'w'), indent="    ", addindent="  ", newl='\n')
	...
	def _save_current_pool(self):
		name = 'gen/neat_gen_' + str(self.pool["generation"]) + '.json'
